...
namespace Rocky.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Nieoczekiwany błąd podczas usuwania użytkownika z ID '{userId}'.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("Użytkownik z ID '{UserId}' usunął swoje konto.", userId);
